...
export default function LatestStakes(props: Props) {
    const { data: stakes, isLoading: stakes_loading } = useStakes({chainId: props.chainId, subcourtID: props.courtId, jurorID: props.jurorId});
    const columns_stakes = [
      ...
      const columns_stakes_for_juror = [
        ...
        {
          field: 'newTotalStake', headerName: 'Total in Courts', type:'number', flex: 1, valueFormatter: (params: { value: BigNumberish }) => {
            return formatPNK(params.value);
          }
        },
        {
    ...
    return (
        <Box>
            ...
            {<DataGrid
                sx={{ marginTop: '30px' }}
                rows={stakes ? stakes! : []}
                columns={props.courtId ? columns_stakes_wihtout_court: props.jurorId? columns_stakes_for_juror : columns_stakes}
