...
export default function PeriodStatus(props: Props) {
  ...
  return (
    ...
    >
      <Stepper
        activeStep={getPeriodNumber(props.currentPeriod) - offsetPeriod}
        orientation={mobile ? 'vertical' : 'horizontal'}
      >
        <Step key="evidence" completed={period > 0}>
          <StepLabel optional={formatPeriod(props.court.timePeriods[0])}>
            <Typography>Evidence Period</Typography>
          </StepLabel>
        </Step>
        {props.court.hiddenVotes ? (
          <Step key="commit" completed={period > 1}>
            <StepLabel optional={formatPeriod(props.court.timePeriods[1])}>
              <Typography>Commit Period</Typography>
            </StepLabel>
          </Step>
        ) : null}
        <Step key="voting" completed={period > 2}>
          <StepLabel optional={formatPeriod(props.court.timePeriods[2])}>
            <Typography>Voting Period</Typography>
          </StepLabel>
        </Step>
        <Step key="appeal" completed={period > 3}>
          <StepLabel optional={formatPeriod(props.court.timePeriods[3])}>
            <Typography>Appeal Period</Typography>
          </StepLabel>
        </Step>
        <Step key="execution" completed={period === 4}>
          <StepLabel optional="Final Decision">
